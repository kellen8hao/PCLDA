...
run_pclda <- function(train_matrix, test_matrix, cell_label, test_y, num_of_pc = 200, scale_data = TRUE，threshold=0.5) {
  start=Sys.time()
  ...
  posterior_probs <- lda.pred$posterior
  max_probs <- apply(posterior_probs, 1, max)
  
  ### Define a threshold for unknown prediction
  threshold <- threshold
  
  # Assign "unknown" where max probability < threshold
  predicted_labels[max_probs < threshold] <- "unknown"
...
result <- pclda_classifier(train_matrix, test_matrix, cell_label, test_y, num_of_pc = 200, scale_data = TRUE，threshold=0.5)
